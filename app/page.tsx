    ...
    <div className="min-h-screen bg-[#0A0F1C] overflow-hidden">
            ...
            {features.map((feature, i) => (
              ...
              >
                ...
                <h3 className="text-xl font-semibold text-white mb-3 flex items-center gap-2">
                  {feature.title}
                  <ChevronRight className="w-5 h-5 text-slate-500 opacity-0 group-hover:opacity-100 
      ...
      <section id="analytics" className="relative z-10 px-6 lg:px-12 py-24">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              Five Powerful
              <span className="text-gradient"> Analytics Dashboards</span>
            </h2>
            <p className="text-slate-400 text-lg max-w-2xl mx-auto">
              Comprehensive insights across operations, maintenance, workforce, 
              consumables, and health & safety.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 lg:grid-cols-5 gap-4">
            {[
              { name: 'Operation', icon: BarChart3, color: 'blue', desc: 'ROP & Performance' },
              { name: 'Maintenance', icon: Shield, color: 'amber', desc: 'Equipment Health' },
              { name: 'Driller & Crew', icon: Activity, color: 'green', desc: 'Team Analytics' },
              { name: 'Consumables', icon: Zap, color: 'purple', desc: 'Resource Usage' },
              { name: 'HSC', icon: Brain, color: 'red', desc: 'Safety Metrics' }
            ].map((dash, i) => (
              <div key={i} className="p-6 rounded-2xl bg-[#111827]/50 border border-[#1E293B]/50 
                                      hover:border-[#0066FF]/30 transition-all duration-300 text-center">
                <div className={`w-12 h-12 rounded-xl bg-${dash.color}-500/20 flex items-center justify-center mx-auto mb-4`}>
                  <dash.icon className={`w-6 h-6 text-${dash.color}-400`} />
                </div>
                <h4 className="font-semibold text-white mb-1">{dash.name}</h4>
                <p className="text-xs text-slate-400">{dash.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative z-10 px-6 lg:px-12 py-24">
        <div className="max-w-4xl mx-auto text-center">
          <div className="p-12 rounded-3xl bg-gradient-to-br from-[#0066FF]/20 via-[#111827] to-[#00D4AA]/10 
                          border border-[#0066FF]/20 backdrop-blur-xl">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              Ready to Transform Your
              <span className="text-gradient"> Drilling Operations?</span>
            </h2>
            <p className="text-slate-400 text-lg mb-8 max-w-xl mx-auto">
              Join leading mining companies using XPLORIX to optimize performance, 
              reduce downtime, and increase profitability.
            </p>
            <Link 
              href="/auth/register"
              className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-[#0066FF] to-[#0052CC] 
            ...
            <p className="text-sm text-slate-500 mt-4">15-day free trial • No credit card required</p>
          </div>
        </div>
      </section>
      ...
      <footer className="relative z-10 px-6 lg:px-12 py-12 border-t border-[#1E293B]/50">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <p className="text-slate-500 text-sm">
            © 2024 XPLORIX. All rights reserved. Premium Drilling Intelligence Platform.
          </p>
        </div>
      </footer>
    </div>
